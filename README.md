...
Um webapp minimalista que baixa o áudio de um vídeo do YouTube, divide em pedaços seguros (<25 MB), manda cada pedaço para a API de transcrição (Whisper) da OpenAI e devolve o texto puro retornado pelo modelo. Ideal para gerar legendas ou arquivos .txt direto do áudio, mesmo em vídeos longos.

## Requisitos

- `node` (>=18)
- `yt-dlp` (instalado via Homebrew ou similar)
